...
export default function HomePage() {
  return (
    <>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-12 pb-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-8">
          <div>
            ...
            <p className="text-[#A8A49E] text-lg leading-relaxed mb-6 max-w-2xl">
              {lead.excerpt}
            </p>
            <div className="flex flex-wrap gap-4 items-center mb-6 text-xs font-mono uppercase tracking-wider">
              <span className="text-[#F4B63D] font-semibold">{lead.category}</span>
              <span className="text-[#6B6862]">{lead.desk}</span>
              <span className="text-[#6B6862]">{lead.date}</span>
              <span className="text-[#6B6862]">{lead.readTime}</span>
            </div>
            <Link href={`/signal/${lead.slug}`} className="inline-flex items-center gap-2 text-[#F4B63D] font-semibold text-sm border-b border-[#8A6420] pb-0.5 hover:border-[#F4B63D] transition-colors">
              Read the Brief
              <svg width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M3 8h10M9 4l4 4-4 4"/></svg>
            </Link>
          </div>
          <div className="border-l border-[rgba(240,237,232,0.08)] pl-6 max-lg:border-l-0 max-lg:pl-0 max-lg:border-t max-lg:pt-6">
            <h3 className="font-mono text-[11px] tracking-[0.12em] uppercase text-[#A8A49E] mb-4 font-medium">
              Latest Signals
            </h3>
            {signals.slice(0, 4).map((s, i) => (
              <div key={i} className="py-3 border-b border-[rgba(240,237,232,0.08)] last:border-b-0">
                <div className="font-mono text-[10px] tracking-[0.1em] uppercase font-medium mb-1" style={{ color: s.deskColor }}>{s.desk}</div>
                <div className="font-serif text-[15px] font-semibold leading-snug text-[#F0EDE8] hover:text-[#F4B63D] transition-colors cursor-pointer mb-1">{s.title}</div>
                <div className="text-[11px] text-[#6B6862] uppercase tracking-wider">{s.date}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* TICKER */}
      <div className="bg-[#0A0D12] border-y border-[rgba(240,237,232,0.08)] py-3 overflow-hidden">
        <div className="flex items-center gap-8 whitespace-nowrap animate-ticker">
          <span className="font-mono text-[10px] tracking-[0.1em] uppercase text-[#F4B63D] font-semibold px-4 shrink-0">Tracking Now</span>
          {[...tickerItems, ...tickerItems].map((item, i) => (
            <span key={i} className="text-xs text-[#A8A49E] shrink-0 flex items-center gap-3">
      ...
      {/* FOUNDER QUOTE */}
      <div className="bg-[#0F1318] border-y border-[rgba(240,237,232,0.08)]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 flex flex-col lg:flex-row items-center gap-8">
          <p className="font-serif text-xl sm:text-2xl italic text-[#F0EDE8] leading-relaxed flex-1 border-l-2 border-[#F4B63D] pl-6 max-lg:border-l-0 max-lg:pl-0 max-lg:border-t-2 max-lg:pt-4 max-lg:text-center">
          ...
          <div className="shrink-0 text-right max-lg:text-center">
            <div className="text-sm font-semibold text-[#F0EDE8]">Keke Lebaka</div>
            <div className="text-[11px] text-[#6B6862] leading-relaxed mt-1 max-w-[260px]">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="font-mono text-[11px] tracking-[0.12em] uppercase text-[#F4B63D]">The Platform</span>
        </div>
        <h2 className="font-serif text-2xl sm:text-3xl font-bold mb-2">One Platform. Three Pillars.</h2>
        <p className="text-[#A8A49E] text-sm mb-10 max-w-xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {[
            ...
            <div key={i} className="bg-[#111520] border border-[rgba(240,237,232,0.08)] rounded-lg p-6 card-hover relative overflow-hidden">
              ...
              <p className="text-sm text-[#A8A49E] leading-relaxed mb-4">{p.desc}</p>
              <ul className="space-y-1.5 mb-5">
                {p.items.map((item, j) => (
                  <li key={j} className="text-xs text-[#6B6862] flex items-center gap-2">
                    <span className="w-1 h-1 rounded-full shrink-0" style={{ background: p.color }} />
                    {item}
                  </li>
                ))}
              </ul>
              <Link href={p.link} className="text-xs font-semibold uppercase tracking-wider text-[#F4B63D] hover:underline">
                {p.linkText} &rarr;
              </Link>
            </div>
          ))}
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="flex items-center gap-2 mb-2">
          <span className="w-4 h-px bg-[#F4B63D]" />
          <span className="font-mono text-[11px] tracking-[0.12em] uppercase text-[#F4B63D]">Coverage</span>
        </div>
        <h2 className="font-serif text-2xl sm:text-3xl font-bold mb-2">Editorial Desks</h2>
        <p className="text-[#A8A49E] text-sm mb-10 max-w-xl">We track the structural forces shaping Africa&apos;s AI future through six dedicated desks.</p>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-px bg-[rgba(240,237,232,0.08)] rounded-lg overflow-hidden">
          {editorialDesks.map((d) => (
            <div key={d.id} className="bg-[#111520] p-5 hover:bg-[#161C24] transition-colors">
              ...
              <h3 className="text-sm font-semibold text-[#F0EDE8] mb-1">{d.name}</h3>
              <p className="text-xs text-[#6B6862] leading-relaxed">{d.description}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <div className="bg-[#0F1318] border-y border-[rgba(240,237,232,0.08)]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="flex items-center gap-2 mb-2">
            <span className="w-4 h-px bg-[#F4B63D]" />
            <span className="font-mono text-[11px] tracking-[0.12em] uppercase text-[#F4B63D]">Flagship Intelligence</span>
          </div>
          <h2 className="font-serif text-2xl sm:text-3xl font-bold mb-2">The Africa AI Index</h2>
          <p className="text-[#A8A49E] text-sm mb-10 max-w-xl">Tracking AI readiness, governance capacity, compute access, talent density, and institutional development across African nations.</p>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
            <div>
              <p className="text-sm text-[#A8A49E] leading-relaxed mb-4">
              ...
              <p className="text-sm text-[#6B6862] mb-6">
                <strong className="text-[#F0EDE8]">Methodology:</strong> Composite scoring across Policy Readiness, Compute Capacity, Talent Pipeline, Institutional Strength, and Application Ecosystem.
              </p>
              <Link href="/ai-index" className="text-sm font-semibold text-[#F4B63D] uppercase tracking-wider hover:underline">
                Explore the Index &rarr;
              </Link>
            </div>
            <div className="grid grid-cols-2 gap-3">
              {countries.map((c) => (
                <div key={c.name} className="bg-[#111520] border border-[rgba(240,237,232,0.08)] rounded-md p-4 hover:border-[rgba(244,182,61,0.25)] transition-colors">
                  ...
                  <div className="flex gap-3">
                    {[
                      ...
                      <div key={m.label} className="text-center">
                        <div className="font-mono text-base font-bold text-[#F4B63D]">{m.val}</div>
                        <div className="text-[9px] text-[#6B6862] uppercase tracking-wider">{m.label}</div>
                      </div>
                    ))}
                  </div>
                </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="flex items-center gap-2 mb-2">
          <span className="w-4 h-px bg-[#F4B63D]" />
          <span className="font-mono text-[11px] tracking-[0.12em] uppercase text-[#F4B63D]">Constitutional Observatory</span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {constitutionalDesks.map((d) => (
            <div key={d.id} className="bg-[#111520] border border-[rgba(240,237,232,0.08)] rounded-md p-4 flex items-start gap-3 hover:border-[rgba(34,197,94,0.3)] transition-colors">
              ...
              <div>
                ...
                <div className="text-xs text-[#6B6862] leading-relaxed mb-1">{d.finding}</div>
                <div className="font-mono text-[10px] text-[#A8A49E] uppercase tracking-wider">
        ...
        <div className="mt-6">
          <Link href="/constitutional" className="text-sm font-semibold text-[#F4B63D] uppercase tracking-wider hover:underline">
      ...
      {/* NEWSLETTER */}
      <div className="bg-[#0A0D12] border-y border-[rgba(240,237,232,0.08)]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 text-center">
          <h2 className="font-serif text-2xl font-bold mb-2">The Good AI Signal</h2>
          <p className="text-[#A8A49E] text-sm mb-8 max-w-md mx-auto">
            A strategic briefing on the institutions, infrastructure, and power dynamics shaping AI in Africa. Weekly.
          </p>
          <div className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
            <input type="email" placeholder="Enter your email" className="flex-1 bg-[#0F1318] border border-[rgba(240,237,232,0.08)] rounded px-4 py-3 text-sm text-[#F0EDE8] placeholder:text-[#6B6862] outline-none focus:border-[#F4B63D] transition-colors" />
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 flex flex-col lg:flex-row items-center gap-8">
        <div className="flex-1 max-lg:text-center">
          <div className="flex items-center gap-2 mb-2 max-lg:justify-center">
            <span className="w-4 h-px bg-[#F4B63D]" />
            <span className="font-mono text-[11px] tracking-[0.12em] uppercase text-[#F4B63D]">About</span>
          </div>
          <h2 className="font-serif text-2xl font-bold mb-3">Africa&apos;s Intelligence Layer</h2>
          <p className="text-sm text-[#A8A49E] leading-relaxed">
        ...
        <div className="shrink-0">
          <Link href="/about" className="text-[#F4B63D] text-sm font-semibold uppercase tracking-wider border-b border-[#8A6420] pb-0.5 hover:border-[#F4B63D] transition-colors">
