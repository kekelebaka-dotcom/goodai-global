  ...
  return (
    <>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <p className="text-[#A8A49E] text-base leading-relaxed mb-12 max-w-2xl">
        ...
        {/* Methodology */}
        <div className="bg-[#0F1318] border border-[rgba(240,237,232,0.08)] rounded-lg p-8 mb-12">
          <h2 className="font-serif text-xl font-bold mb-6">Methodology: Five Pillars</h2>
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            {pillars.map((p, i) => (
              <div key={p.name} className="text-center">
                <div className="font-mono text-2xl font-bold text-[#F4B63D] mb-2">{i + 1}</div>
                <div className="text-sm font-semibold mb-1">{p.name}</div>
                <div className="text-xs text-[#6B6862] leading-relaxed">{p.desc}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-12">
          {countries.map((c) => (
            <div key={c.name} className="bg-[#111520] border border-[rgba(240,237,232,0.08)] rounded-lg p-5 card-hover">
                ...
                {[
                  ...
                  <div key={m.label}>
                    <div className="flex justify-between text-xs mb-1">
                      <span className="text-[#6B6862]">{m.label}</span>
                      <span className="font-mono text-[#F4B63D]">{m.val}</span>
                    </div>
                    <div className="h-1.5 bg-[#0A0D12] rounded-full overflow-hidden">
